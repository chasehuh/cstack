...
```bash
_UPD=$(~/.agents/skills/gstack/bin/gstack-update-check 2>/dev/null || .agents/skills/gstack/bin/gstack-update-check 2>/dev/null || true)
[ -n "$_UPD" ] && echo "$_UPD"
```

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.agents/skills/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (ask the user directly about the upgrade, upgrade if yes, `touch ~/.gstack/last-update-check` if no). If `JUST_UPGRADED <from> <to>`: tell user "Running gstack v{to} (just updated!)" and continue.
...
Read `.agents/skills/review/greptile-triage.md` and follow the fetch, filter, and classify steps.
...
- If CRITICAL issues found: output all findings, then for EACH critical issue use a separate ask the user directly with the problem, your recommended fix, and options (A: Fix it now, B: Acknowledge, C: False positive — skip).
...
1. **VALID & ACTIONABLE comments:** These are already included in your CRITICAL findings — they follow the same ask the user directly flow (A: Fix it now, B: Acknowledge, C: False positive). If the user chooses C (false positive), post a reply using the appropriate API from the triage doc and save the pattern to `~/.gstack/greptile-history.md` (type: fp).

2. **FALSE POSITIVE comments:** Present each one via ask the user directly:
...
3. **VALID BUT ALREADY FIXED comments:** Reply acknowledging the catch — no ask the user directly needed:
